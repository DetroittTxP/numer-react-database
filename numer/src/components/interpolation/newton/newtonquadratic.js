...
export const Newtonquadratic=()=>{
         ...
         Setdatasets((prev) =>{
            ...
            if(prev.length < size){
                ...
                for(let i =0 ; i<newsize;i++){
                    newdataset.push({x:0,y:0});
                }

            } else{
                newdataset.length = size;
            }     
    ...
    const handleSubmit = async ()=>{
         await axios.post('http://localhost:1234/newtoninterpolation/quadratic',{datasets,x})
         .then((res)=>{
             setresult(res.data);
             Setshowresult(true);
         })
    ...
    return(
        <div>
            <div className="bisection">
                    <Title level={2}>NEWTON DIVIDED DIFFERENCE (QUADRATIC)</Title>
